document.getElementById('form-item').addEventListener('submit', async function(event) {
    ...
    const quantidadeTotal = parseInt(document.getElementById('quantidadeTotal').value, 10);
...
async function deletarItem(id, nome) {
    ...
    try {
        const response = await fetch(`/itens-cautela/${id}`, {
            method: 'DELETE',
            credentials: 'include'
        });
...
function carregarItens() {
    fetch('/itens-cautela', { credentials: 'include' })
        .then(resp => {
            if (resp.status === 401) {
        ...
        .then(itens => {
            ...
            itens.forEach(item => {
                ...
                tr.innerHTML = `
                    ...
                    <td><strong>${item.quantidadeTotal || 1}</strong></td>
